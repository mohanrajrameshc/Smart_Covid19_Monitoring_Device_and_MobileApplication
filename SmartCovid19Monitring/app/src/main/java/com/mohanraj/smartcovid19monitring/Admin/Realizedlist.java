...
import com.mohanraj.smartcovid19monitring.Admin.Report.ReleasedReport;
...
public class Realizedlist extends AppCompatActivity {
    private RecyclerView recview;
    private RealeadedAdapter adapter;
    TextView tv;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_realizedlist);
        ...
        FirebaseRecyclerOptions<RealeaedAdpBean> options =
                new FirebaseRecyclerOptions.Builder<RealeaedAdpBean>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("RELEASED_LIST"), RealeaedAdpBean.class)
                        .build();

        adapter=new RealeadedAdapter(options);
        ...
        report.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i=new Intent(getApplicationContext(), ReleasedReport.class);
                startActivity(i);
            }
        });
    ...
    {
        final FirebaseDatabase database = FirebaseDatabase.getInstance();
        final DatabaseReference ref = (DatabaseReference) database.getReference().child("RELEASED_LIST");
        Query query=ref.orderByChild("userid").startAt(s).endAt(s+"\uf8ff");
